...
def updateStatus():
  """
  Obtiene de sensor el último estado
  Es decir, si se encuentra conectado al broker
  (Más adelante, se le añadirá más información)
  """
  if request.remote_addr != '127.0.0.1':
    return Response(response='Not Authorized', status=401)
  global brokerConnected, lastUpdated, updatedConfig
  brokerConnected = request.args.get('status') == 'True'
  lastUpdated = datetime.datetime.now()
  return Response(status=200, response={'updated' : updatedConfig})


@app.route('/updateConfig', methods=['GET'])
def updateConfigIOT():
  """
  Obtiene la configuración del IOT
  Esta función es llamada por el programa que se encarga de obtener los datos del sensor
  Ya que el administrador, puede cambiar la configuración desde la API
  """
  if request.remote_addr != '127.0.0.1':
    return Response(response='Not Authorized', status=401)
  global updatedConfig
